...
public class MessageHandlerModel : MonoBehaviour
{
    ...
    private async UniTask Handle()
    {
        ...
        if (PlayerPrefs.HasKey(SavingConfig.AUTHORIZATION_HANDLER_IS_COMPLEATE) == false)
        {
            Debug.Log("Fetch auth...");
            message = await _remoteConfig.GetValueAsync("AuthorizationHeader");

            PlayerPrefs.SetString(SavingConfig.AUTHORIZATION_HANDLER_IS_COMPLEATE, message);
        }
        else
        {
            message = PlayerPrefs.GetString(SavingConfig.AUTHORIZATION_HANDLER_IS_COMPLEATE);
        }
